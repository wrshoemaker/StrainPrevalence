...
def make_plot(variant_type):
    ...
    for species_name in species_list_sites_to_plot:
        ...
        # save to dictionary
        rescaled_dist_dict[species_name] = {}
        rescaled_dist_dict[species_name]['afd'] = f_no_zeros_mapgd_log10_rescaled
        rescaled_dist_dict[species_name]['mad'] = f_mean_mapgd_log10_rescaled
    ...
    shape_gamma, loc_gamma, scale_gamma = stats.loggamma.fit(f_no_zeros_mapgd_log10_rescaled_all)
    ...
    # Gaussian fit
    shape_norm, scale_norm = stats.norm.fit(f_no_zeros_mapgd_log10_rescaled_all)
    ...
    aic_loggamma = 6 - 2*ll_loggamma
    aic_normal = 4 - 2*ll_norm

    # difference 
    #print("AIC(gamma) - AIC(lognormal) = ",  )
    print("AIC(gamma) = ", aic_loggamma)
    print("AIC(lognormal) = ", aic_normal)

    #np.log(np.product(norm.pdf(data,m,s)))
    ...
    if len(means_all) > 0:
        ...
        if len(means_log10_all_test) > 0:
            ...
            ax_f_mean_vs_var.plot(10**x_log10_range, 10**y_log10_fit_range, c='k', lw=4, linestyle='--', zorder=2, label=r'$\sigma^{{2}}_{{f}} \sim \bar{{f}}\,^{{{}}}$'.format(round(slope, 2)))
    ...
    ax_f_mean_vs_var.plot(x_inequality, y_inequality, c='k', lw=4, linestyle=':', zorder=1, label='Max. ' + r'$\sigma_{f}^{2}$')
    ...
    # plot CDF of AFD and MAD..

    fig_cdf = plt.figure(figsize = (8, 4)) #

    ax_afd = plt.subplot2grid((1, 2), (0, 0), colspan=1)
    ax_mad = plt.subplot2grid((1, 2), (0, 1), colspan=1)

    x_range_afd = numpy.linspace(-6, 4, num=200, endpoint=True)
    x_range_mad = numpy.linspace(-6, 4, num=200, endpoint=True)

    for species_name in species_list_sites_to_plot:

        afd = rescaled_dist_dict[species_name]['afd']
        mad = rescaled_dist_dict[species_name]['mad']

        survival_afd = [sum(afd >= i)/len(afd) for i in x_range_afd]
        survival_mad = [sum(mad >= i)/len(mad) for i in x_range_mad]

        survival_afd = numpy.asarray(survival_afd)
        survival_mad = numpy.asarray(survival_mad)

        idx_to_keep_afd = (survival_afd > 0)
        x_range_afd_to_plot = x_range_afd[idx_to_keep_afd]
        survival_afd_to_plot = survival_afd[idx_to_keep_afd]

        idx_to_keep_mad = (survival_mad > 0)
        x_range_mad_to_plot = x_range_mad[idx_to_keep_mad]
        survival_mad_to_plot = survival_mad[idx_to_keep_mad]

        ax_afd.plot(x_range_afd_to_plot, survival_afd_to_plot, ls='-', lw=3, c=species_color_map[species_name], zorder=2, alpha=0.8)
        ax_mad.plot(x_range_mad_to_plot, survival_mad_to_plot, ls='-', lw=3, c=species_color_map[species_name], zorder=2, alpha=0.8)


    # CDF
    cdf_loggamma = stats.loggamma.sf(x, shape_gamma, loc=loc_gamma, scale=scale_gamma)
    cdf_norm = stats.norm.sf(x_norm, shape_norm, scale_norm)

    ax_afd.plot(x, cdf_loggamma, 'k', ls='--', lw=4, label='Gamma')
    ax_afd.plot(x_norm, cdf_norm, 'k', ls=':', lw=4, label='Lognormal')

    ax_afd.set_yscale('log', basey=10)
    ax_mad.set_yscale('log', basey=10)

    ax_afd.set_xlabel('Rescaled ' + r'$\mathrm{log}_{10}$' + ' within-host allele frequency', fontsize=11)
    ax_mad.set_xlabel('Rescaled ' + r'$\mathrm{log}_{10}$' + ' mean within-host\nallele frequency across hosts', fontsize=11)

    ax_afd.set_ylabel('Survival probability', fontsize=12)
    ax_afd.legend(loc='lower left', fontsize=11)
    ax_mad.set_ylabel('Survival probability', fontsize=12)
    ...
    ax_mad.text(-0.1, 1.04, plot_utils.sub_plot_labels[1], fontsize=10, fontweight='bold', ha='center', va='center', transform=ax_mad.transAxes)


    fig_cdf.tight_layout()
    fig_cdf.subplots_adjust(wspace=0.34, hspace=0.28)
    fig_cdf.savefig("%sdiversity_summary_survival_%s.pdf" % (config.analysis_directory, variant_type), format='pdf', bbox_inches = "tight", pad_inches = 0.2, dpi = 600)
    plt.close()
